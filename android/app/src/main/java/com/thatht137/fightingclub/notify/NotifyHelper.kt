...
import com.thatht137.fightingclub.BuildConfig
 ...
 *
 * 使用 OkHttp 发起 HTTP 请求，Cookie 存储与主程序�?java.net.CookieManager
 * �?android.webkit.CookieManager 完全隔离，互不干扰�?
 *
 * 每次 Worker 运行时都使用 CASTGC 重新建立 JWXT 会话，不依赖持久化的 session cookies�?
 * 学校配置�?UnifiedCache 读取，支�?JS 端动态下发�?
 */
...
object NotifyHelper {
     ...
     *
     * 流程�?
     * 1. 请求 CAS（带 CASTGC），CAS 返回 302 �?JWXT（带 ticket�?
     * 2. 手动跟随 CAS �?JWXT 重定向（OkHttp 跨域会剥�?Cookie�?
     * 3. JWXT 返回 302（带 Set-Cookie: GS_SESSIONID 等）�?*不再跟随此重定向**
     * 4. CookieJar 已存入会�?cookie，可直接用于后续请求
     *
    ...
    fun establishSession(context: Context, castgc: String): Boolean {
        ...
        try {
            client.newCall(buildGet(casUrl)).execute().use { resp ->
                ...
                if (resp.code !in setOf(301, 302, 303, 307, 308)) {
                    ...
                    // CAS 直接返回 200（已有有�?session），但我们需�?JWXT �?cookie
                    // 继续请求 JWXT portal
                }
        ...
        try {
            client.newCall(buildGet(casRespUrl)).execute().use { resp ->
                ...
                if (resp.code in setOf(301, 302, 303, 307, 308)) {
                    // 302 是预期的（重定向到不�?ticket �?index.do），cookie 已在 CookieJar �?
                    return true
                }
    ...
    /** 访问 appShow.do 获取指定应用�?_WEU cookie�?*/
    ...
    /**
     * �?mappings 中解�?raw key。JS 端可能发送字符串或字符串数组（优先候选列表）�?
     */
    ...
    /**
     * 将原始成绩对象转换为标准格式，字段名�?server config 中的 fieldMappings 定义�?
     */
